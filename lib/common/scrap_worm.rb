...
module ScrapWorm
   ...
   def scrap_sexi_res(options={})  
     cartoons_arr = []
     begin
      sexi = ScrapSource.where('name=?', options[:name]).first_or_create!(
        name: options[:name], 
        src_url: options[:url],
        category_id: options[:category_id]
      )

      if sexi.total_pages.nil? || sexi.curr_page <= sexi.total_pages
        curr_scrap_page = sexi.curr_page
        if sexi.curr_page.nil? || sexi.curr_page == 1
          #the first page
          page = Nokogiri::HTML(open(sexi.src_url))
          sexi.curr_page = 2
        else
          #from page 2 start
          page = Nokogiri::HTML(open("http://xiaojiulou.com/sexi/list_#{curr_scrap_page}.html"))
          sexi.curr_page += 1
        end

        #get total pages
        total_pages = page.css("div.pages>strong")[0].text
        sexi.total_pages = total_pages if sexi.total_pages != total_pages
        # when current page nodes saved, then update scrap source object
        sexi.save 
        #the first page per cartoon, it should be 18           
        nodes = page.css("div.paihang>ul li")   
   ...
   def save_sexiaozu(cartoon_tmp_id)
    begin
      ...
      cartoon = Cartoon.create({
        ...
        scrap_source_id: cartoon_tmp.scrap_source_id
      })
